...
import { CountryContext as MapCountryContext } from "react-svg-worldmap";
...
export const FriendsTab: FC = () => {
  ...
  const filteredPhotos = useMemo<Photo[]>(
    () =>
      photoFilter
        ? friendsPhotos.filter(
            (photo) =>
              photo.countryCode.toLowerCase() === photoFilter.toLowerCase()
          )
        : friendsPhotos,
  ...
  useEffect(() => {
    ...
    countries.forEach((dataItem: ApiCountry) => {
      const count = friendsPhotos.filter(
        (photo) => photo.countryCode === dataItem.code
      ).length;
      if (count > 0) {
        countryData.push({
