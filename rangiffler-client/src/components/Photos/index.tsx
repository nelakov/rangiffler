...
export const Photos: FC<PhotosInterface> = ({ photos, handlePhotoClick }) => {
  ...
  return (
    <Box
      sx={{ margin: "0 auto", width: 1200, height: 650, overflowY: "scroll" }}
    >
      <ImageList cols={4} gap={14}>
        {photos.map((item) => (
          <ImageListItem
            key={item.id}
            sx={{
          ...
          >
            <Box>
              <Button onClick={() => handlePhotoClick(item)}>
                <img
              ...
              <span>
                <Typography sx={{ padding: "12px", textAlign: "center" }}>
                  <PlaceIcon sx={{ verticalAlign: "bottom" }} />
                  {
                    countries.find(
                      (country) => country.code === item.countryCode
                    )?.name
                  }
                </Typography>
              </span>
