...
- **Web e2e frontend**: serve with `npm run start:e2e` at :3001, NOT `npm start`. `start:e2e` = `NODE_ENV=e2e` (→ `.env.e2e` localhost backend; `IS_DEV` false → no react-refresh; `IS_E2E` → `devServer.client.overlay:false` + `hot:false`) and **`--mode development`**. Two reasons it must NOT be `--mode production`: (1) MUI strips the dev-only `data-testid` on icons in production builds, so selectors like `svg[data-testid='PublicIcon']` find nothing; (2) the click-intercepting overlays (react-refresh + webpack "Compiled with problems") must be off. Dev mode keeps `data-testid`; `IS_DEV`/`IS_E2E` keep both overlays off. Page-object selectors must use stable hooks (`data-testid`, roles) — never MUI emotion hashes (`css-x0sd41`), which change on every MUI upgrade.
- **Structured logging is `docker`-profile only**: backend services emit ECS JSON only under the `docker` profile; `local`/default stays plain text. To get JSON locally, override per run: `--logging.structured.format.console=ecs`. Native Boot 4 (no deps); the `logging.structured` block lives inside each service's `docker` profile doc, before the closing `---`.
- **gRPC request tracing**: a `requestId` rides `X-Request-Id` (gateway) → gRPC metadata `x-request-id` → MDC → ECS `requestId` field across country/userdata/photo. Shared contract in `rangiffler-grpc-common` (`tracing/RequestIdSupport`); interceptors wired per service via `@GlobalServerInterceptor`/`@GlobalClientInterceptor` (spring-grpc). Server interceptor binds MDC around **each** listener callback (gRPC executor thread-hops); `grpc.health.*`/`grpc.reflection.*` are skipped from the per-call access log to avoid probe flooding.
- **rangiffler-grpc-common is `java-library`**: its grpc bundle + slf4j are `api`-scoped so the generated stubs and shared `tracing/*` classes are visible to consumers. To add a shared class there that downstream must see, use `api` not `implementation` — and note the plain `java` plugin has no `api` config (needs `java-library`).
