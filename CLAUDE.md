...
- **Web e2e frontend**: serve with `npm run start:e2e` (production-mode webpack at :3001), NOT `npm start`. `start:e2e` uses `NODE_ENV=e2e` → `.env.e2e` (localhost backend), production mode (no react-refresh), and **`devServer.client.overlay:false` + `hot:false`** (only for e2e). Both overlays intercept Selenide clicks → false `element click intercepted` failures: the react-refresh overlay AND webpack's "Compiled with problems" overlay (shown for the bundle-size warning). Disabling both is required for web tests to interact with the page.
